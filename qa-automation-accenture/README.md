...
3 - Validar autorização do usuário
...
Observação: testes de API não possuem saída visual.
O sucesso é indicado por BUILD SUCCESS.
...
mvn test -Dcucumber.filter.tags=@practiceform
mvn test -Dcucumber.filter.tags=@webtables
mvn test -Dcucumber.filter.tags=@sortable
mvn test -Dcucumber.filter.tags=@browserwindows
mvn test -Dcucumber.filter.tags=@progressbar
